...
object KipInputConverter {
    private val TAG = KipInputConverter::class.java.simpleName

    private val sKipWordExtractPattern = Pattern.compile("(?:(ph|p|m|b|th|tsh|ts|t|l|kh|k|ng|n|g|h|s|j)?([aiueo+]+(?:nn)?|ng|m)(?:(ng|m|n|re|r)|(p|t|h|k))?([12345789])?|(ph|p|m|b|th|tsh|ts|t|l|kh|k|ng|n|g|h|s|j)-?-?)", Pattern.CASE_INSENSITIVE)

    fun convertKipNumberRawInputToTailoWords(input: String?): String? {
        ...
        val matcher = sKipWordExtractPattern.matcher(input)
        ...
        while (matcher.find()) {
            ...
            val kip = convertTailoNumberToTailo(foundTaigiWord)
            if (BuildConfig.DEBUG_LOG) {
                Log.d(TAG, "foundTaigiWord=$foundTaigiWord, kip=$kip")
            }

            stringBuilder.append(kip)
    ...
    private fun convertTailoNumberToTailo(kipNumber: String): String {
        val fixedTailoNumber = ConverterUtils.fixLomajiNumber(kipNumber)
    ...
    private fun replaceTailoNumberWithTailoUnicode(number: String, tailoWithoutNumber: String, contains: String): String {
        ...
        val tailoUnicode = Kip.sKiplmjNumberToKiplmjUnicodeHashMap[tailoCharNumber]
